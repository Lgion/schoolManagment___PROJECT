...
test.describe('RBAC Middleware Protection (E2E)', () => {
    test.describe('Unauthorized user', () => {
        test('is redirected to sign-in when accessing admin dashboard', async ({ browser, log }) => {
            await log.step('Given an unauthenticated user');
            // Create a completely clean, cookie-free browser context
            const context = await browser.newContext({ storageState: { cookies: [], origins: [] } });
            const page = await context.newPage();

            // Mock all API requests to prevent server database timeouts
